...
public class DemoBoxCommand {
	...
	private static RequiredArgumentBuilder<CommandSourceStack, Identifier> buildArgTree(CommandHandler handler) {
		return argument("template", IdentifierArgument.id())
			.suggests(STRUCTURE_SUGGESTION_PROVIDER)
			.executes(context -> handler.execute(context, IdentifierArgument.getId(context, "template"), new Vec3(0.5, 2, 0.5), List.of(), List.of()))
			.then(argument("pos", Vec3Argument.vec3())
				.executes(context -> handler.execute(context, IdentifierArgument.getId(context, "template"), Vec3Argument.getVec3(context, "pos"), List.of(), List.of()))
				.then(argument("setupFunction", FunctionArgument.functions())
					.suggests(FunctionCommand.SUGGEST_FUNCTION)
					.executes(context -> handler.execute(context, IdentifierArgument.getId(context, "template"), Vec3Argument.getVec3(context, "pos"), FunctionArgument.getFunctions(context, "setupFunction"), List.of()))
					.then(
						argument("playerFunction", FunctionArgument.functions())
							.suggests(FunctionCommand.SUGGEST_FUNCTION)
							.executes(context -> handler.execute(context, IdentifierArgument.getId(context, "template"), Vec3Argument.getVec3(context, "pos"), FunctionArgument.getFunctions(context, "setupFunction"), FunctionArgument.getFunctions(context, "playerFunction"))))));
	}
	...
	private static int executeSign(CommandContext<CommandSourceStack> context, Identifier structure, Vec3 pos, Collection<CommandFunction<CommandSourceStack>> functions, Collection<CommandFunction<CommandSourceStack>> playerFunctions) throws CommandSyntaxException {
	...
	private static int executeOpen(CommandContext<CommandSourceStack> context, Identifier structure, Vec3 pos, Collection<CommandFunction<CommandSourceStack>> functions, Collection<CommandFunction<CommandSourceStack>> playerFunctions) throws CommandSyntaxException {
		var source = context.getSource();
		var player = source.getPlayerOrException();
